...
// 使用三元组累加所有元件导纳贡献；相同位置自动合并（Eigen 会汇总重复条目）
void Grid::buildMatrix(double dt)
{
    ...
    // 全局 gmin（避免孤立/近孤立的超级结点导致奇异）
    const double gmin_global = 1e-9; // 也可 1e-10~1e-8 之间微调
    for (int i = 0; i < num_nodes; ++i)
        triplets.emplace_back(i, i, gmin_global);

    // 根据最新的节点总数（可能在allocateInternalNodes后改变）重置矩阵大小
    G.resize(num_nodes, num_nodes);
    // 从三元组列表构建稀疏导纳矩阵G。相同位置的元素会自动累加。
    G.setFromTriplets(triplets.begin(), triplets.end());
    G.makeCompressed(); // 压缩存储格式，优化后续运算
}
