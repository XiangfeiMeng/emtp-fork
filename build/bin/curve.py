...
"""
curve.py
默认：在脚本同目录下读取 'curve_V.dat'、'curve_I.dat'、'curve_S.dat'，
     分别各自绘图与保存，存在哪个就绘哪个，然后一次性 show() 打开所有窗口。
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="绘制 curve_V.dat / curve_I.dat / curve_S.dat（存在哪个就绘哪个）。兼容 -i 单文件模式。"
    )
    parser.add_argument("-i", "--input", type=Path, default=None,
                        help="单文件模式：指定一个数据文件（curve_V.dat / curve_I.dat / curve_S.dat / 旧版 curve.dat）")
    ...
    parser.add_argument("--titleI", type=str, default="Currents",
                        help="电流图标题")
    parser.add_argument("--titleS", type=str, default="P / Q (Converter Sampling)",
                        help="功率图标题（curve_S.dat）")
    ...
    if args.input is not None:
        ...
        # 自动根据文件名设置默认标题
        default_title = None
        fn = args.input.name.lower()
        if "curve_v" in fn:
            default_title = args.titleV
        elif "curve_i" in fn:
            default_title = args.titleI
        elif "curve_s" in fn:
            default_title = args.titleS
        ...
        if s_path.exists():
            figs.append(
                plot_file(
                    s_path,
                    out_png=script_dir / "curve_S.png",
                    dpi=args.dpi,
                    title=args.titleS,
                    xcol_hint=args.xcol,
                    window_title="P/Q (curve_S.dat)",
                )
            )
            any_plotted = True
        else:
            print("ℹ️ 未找到 curve_S.dat，跳过功率图。")
        ...
        if not any_plotted:
            ...
            if legacy.exists():
                ...
                print("❌ 未发现可绘制的数据：curve_V.dat / curve_I.dat / curve_S.dat / curve.dat", file=sys.stderr)
                sys.exit(1)
