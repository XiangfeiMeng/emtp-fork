/*
 * 数值离散总览（梯形法 → 诺顿等效）：
 *  - 电感 L：G_eq = dt/(2L)，I_hist ← I_hist + 2*G_eq*v_prev，i_now = I_hist + G_eq*v_now
 *  - 电容 C：G_eq = 2C/dt    ，I_hist ← -I_hist - 2*G_eq*v_prev，i_now = I_hist + G_eq*v_now
 *  - 串联 RL：等效并联导纳 G_eq = 1/(R + 2L/dt)，历史项采用上一拍电压/电流递推（详见 series_RL）
 * 方向约定：设备 stamp 时按节点电压顺序定义支路电压 v = V(n1)-V(n2)，电流正向由 n1 指向 n2。
 * 单位：kV/kA/Ω/H/F/Hz；因此导纳/电流注入在数值上与 SI 保持一致。
 * ====== */
...
// --- series_RL ---
// TRAP 离散要点：串联 RL 的诺顿化
//   i(t) = G_eq·v(t) + I_hist(t)
// 其中 G_eq = 1/(R + 2L/dt)，I_hist 递推保留电磁能量，数值上等价于“π等效”形式。
// 该写法兼顾直流稳态与交流稳态的无偏性，数值阻尼适中
...
// --- Inductor ---
// 电感的历史注入：I_hist(t) = i(t-dt) + (dt/2L)·v(t-dt)
// 与 TRAP 的“等效电导 + 历史电流”严格一致；与上一步支路电压 v_hist 成正比。
...
// --- Capacitor ---
// 电容的历史注入：I_hist(t) = -i(t-dt) - (2C/dt)·v(t-dt)
// 注意符号与电感相反；电容并联等效导纳为 2C/dt（两端各一半为 C/dt）。
...
// --- VoltageSource ---
// 理想电压源 + 戴维南内阻/短路电感 经诺顿化后的注流：I_src = v_src / Rs
// v_src 允许在运行中叠加幅值缩放与频率偏移（支持电压/频率事件）
// 串联 L 与并联 R 的历史项分别交由内部器件 updateHistory()
...
void VoltageSource::updateHistory(Eigen::VectorXd& I, double t, double dt)
{
    ...
    if (rs_to_gnd)
        rs_to_gnd->updateHistory(I, t, dt);
    if (series_L)
...
// --- PI_line ---
/* 3×3 相域 PI 线路离散：
 * - 串联支路：G_series = (Rabc + (2/dt)Labc)^{-1}
 * - 并联支路（两端各半）：G_sh_half = Cabc/dt
 * - 历史电流衰减矩阵：Alpha = (I - 0.5·dt·L^{-1}R)·(I + 0.5·dt·L^{-1}R)^{-1}
 * 由正/零序参数按完全换位假设生成 Rabc/Labc/Cabc；详见 make_seq_to_phase_real()。
 */
...
// Bergeron 段（i<->k）的历史注入：等效为 (1/zc)·对端电压 + 上拍对端电流
// 使 i 端与 k 端在时间上“交替耦合”，配合 PI-ZIM 部分形成混合模型。
// 注意：该模型为试验性实现，参数域与精度见作者论文
