 ...
 * EMTP 元件基类与标准离散化接口
 * -----------------------------------------------------------------------------
 * 单位体系：V[kV], I[kA], R[Ω], L[H], C[F], f[Hz], P[MW/MVAr]
 * 节点编号：1-based（0 为地），在构造/更新中均需判定 n>0 再访问 V(n-1)
 *
 * 统一约定：
 *   - stamp()：将“当前时间步的等效导纳”写入三元组（稀疏矩阵结构 G）
 *   - updateHistory()：将“历史等效电流源注入”叠加到 I 向量
 *   - updateState()：在解得 V 后，更新元件当前电流/历史电压等内部状态
 *   - allocateNodes()：复合元件申请内部节点（如理想源串入 L、线路内部节点等）
 *
 * 数值方法：储能元件采用梯形积分，均以诺顿等效进入网络方程。
 * ============================================================================= */
...
public:
    ...
    double get_I() { return series_L->get_I(); }
